...
func (_q *StationQuery) QueryConnectors() *ConnectorQuery {
	query := (&ConnectorClient{config: _q.config}).Query()
...
func (_q *StationQuery) QueryPrices() *PriceQuery {
...
func (_q *StationQuery) Clone() *StationQuery {
	...
	return &StationQuery{
		...
		withConnectors: _q.withConnectors.Clone(),
...
func (_q *StationQuery) WithConnectors(opts ...func(*ConnectorQuery)) *StationQuery {
	query := (&ConnectorClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withConnectors = query
	return _q
}
...
func (_q *StationQuery) WithPrices(opts ...func(*PriceQuery)) *StationQuery {
	query := (&PriceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *StationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Station, error) {
	...
	if query := _q.withConnectors; query != nil {
		if err := _q.loadConnectors(ctx, query, nodes,
			func(n *Station) { n.Edges.Connectors = []*Connector{} },
			func(n *Station, e *Connector) { n.Edges.Connectors = append(n.Edges.Connectors, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPrices; query != nil {
		if err := _q.loadPrices(ctx, query, nodes,
			func(n *Station) { n.Edges.Prices = []*Price{} },
			func(n *Station, e *Price) { n.Edges.Prices = append(n.Edges.Prices, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *StationQuery) loadConnectors(ctx context.Context, query *ConnectorQuery, nodes []*Station, init func(*Station), assign func(*Station, *Connector)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(connector.FieldStationID)
	}
	query.Where(predicate.Connector(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(station.ConnectorsColumn), fks...))
	}))
...
func (_q *StationQuery) loadPrices(ctx context.Context, query *PriceQuery, nodes []*Station, init func(*Station), assign func(*Station, *Price)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Station)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(price.FieldStationID)
	}
	query.Where(predicate.Price(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(station.PricesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
