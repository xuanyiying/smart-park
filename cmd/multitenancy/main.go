...
func init() {
	flag.StringVar(&flagconf, "conf", "../../configs/multitenancy.yaml", "config path")
}

func newApp(logger log.Logger, gs *grpc.Server, hs *http.Server) *kratos.App {
	return kratos.New(
		kratos.Name("multitenancy"),
...
func main() {
	...
	tenantRepo := data.NewTenantRepo(dataLayer)

	// Initialize business logic
	tenantUseCase := biz.NewTenantUseCase(tenantRepo, logger)

	// Initialize gRPC service
	tenantSvc := service.NewTenantService(tenantUseCase, logger)
	...
	// Register services
	v1.RegisterTenantServiceServer(gs, tenantSvc)
